...
<script>
    let movieSelector = new Selectr('#moviesActor', {
  ...
  <div class="columns is-marginless is-centered is-mobile">
    <div class="column is-11">
      <div class="columns is-mobile">
        <div class="column">
          <h1 class="title is-3">Actores</h1>
        </div>
        <div class="column is-narrow">
          <a href="{{route('actor.create')}}" class="button is-success">Crear actor</a>
        </div>
      </div>
      ...
      <form class="" action="" method="get">
        ...
        <div id="is-search" class="is-hidden-touch">
          <div class="columns">
            <div class="column">
              <div class="field">
                <label class="label">Nombre del actor</label>
                <div class="control">
            ...
            <div class="column">
                <div class="field">
                    <label class="label">Actua en</label>
                    <div class="control">
                        <div class="is-fullwidth">
                            <select id='moviesActor' name="movies[]" multiple>
          ...
          <div class="columns is-mobile">
            <div class="column">
              <div class="buttons">
                <a href="{{route('actors.index')}}" type="reset" class="button">Restablecer</a>
              ...
              <tr>
                ...
                <td data-label="Opciones">
                  <div class="field is-grouped">
                    <p class="control">
                      <a href="{{route('actor.show', [$actor->actorId])}}" class="button is-outlined is-small" title="Consultar">
                    ...
                    <p class="control">
                      <a href="{{route('actor.edit', [$actor->actorId])}}" class="button is-outlined is-info is-small" title="Editar">
                        <span class="icon is-small">
                    ...
                    <p class="control">
                      <button onclick="deleteItem(event)" class="delete-item is-danger button is-outlined is-small" type="button" name="button" data-route="{{ route('actor.destroy', $actor->actorId)}}" title="Eliminar">
                        <span class="icon is-small">
				...
				<td class="col0"> {{ $actor->actorId }} </td>
				<td class="col1"> {{ $actor->name }} </td>
				<td class="col2"> {{ empty($actor->biography) ? "" : $actor->biography }} </td>
				<td class="col3"> {{ empty($actor->country) ? "" : $actor->country }} </td>
				<td class="col4"> {{ empty($actor->birthdate) ? "" : $actor->birthdate }} </td>
				<td class="col6"> {{ $actor->created_at}} </td>
				<td class="col7"> {{ $actor->updated_at }} </td>
              </tr>
              @endforeach
              @endif
        ...
        <div class="columns is-mobile">
          <div class="column">
            <strong>
              @if ($actors->total()>0) Mostrando {{ $actors->firstItem() }} - {{ $actors->lastItem() }} de @endif {{$actors->total()}} resultados
            </strong>
          </div>
          <div class="column is-narrow">
                  ...
                  <div class="control">
                    <div class="select is-fullwidth is-small">
                      <select name="pagesize" class="">
                        <option value="25" @if ($actors->perPage() == 25) selected @endif>25</option>
                        <option value="50" @if ($actors->perPage() == 50) selected @endif>50</option>
                        <option value="100" @if ($actors->perPage() == 100) selected @endif>100</option>
                      </select>
                    </div>
                  </div>
                </div>
